	...
	<?php do_action( 'single_resume_start' ); ?>
		...
		<div class="eight columns">
			<?php if ( $items = get_post_meta( $post->ID, '_candidate_education', true ) ) : ?>
				<h3 class="margin-bottom-20"><?php esc_html_e( 'Onderwijs', 'workscout' ); ?></h3>
				...
				<?php
					foreach( $items as $item ) : ?>
						...
						<dt>
							...
							<strong><?php printf( esc_html__( '%s %s aan  %s', 'workscout' ), '<span class="qualification">' . esc_html( $item['qualification'] ) . '</span>', '<span class="course">' . esc_html( $item['course'] ) . '</span>', '<span class="location">' . esc_html( $item['location'] ) . '</span>' ); ?></strong> 
						</dt>
						<dd>
			...
			<?php if ( $items = get_post_meta( $post->ID, '_candidate_experience', true ) ) : ?>
				<h3 class="margin-bottom-20"><?php esc_html_e( 'Werkervaring', 'workscout' ); ?></h3>
				...
				<?php
					foreach( $items as $item ) : ?>
						...
						<dt>
							<small class="date"><?php echo esc_html( $item['date'] ); ?></small>
							<strong><?php printf( esc_html__( '%s bij %s', 'workscout' ), '<span class="job_title">' . esc_html( $item['job_title'] ) . '</span>', '<span class="employer">' . esc_html( $item['employer'] ) . '</span>' ); ?></strong> 
						</dt>
						<dd>
			...
			<?php if ( $items = get_post_meta( $post->ID, '_activities', true ) ) : ?>
				<h3 class="margin-bottom-20"><?php esc_html_e( 'Nevenactiviteiten', 'workscout' ); ?></h3>
				<dl class="resume-table resume-manager-activities">
				<?php
					foreach( $items as $item ) : ?>

						<dt>
							<small class="date"><?php echo esc_html( $item['date'] ); ?></small>
							<strong><?php printf( esc_html__( '%s bij %s', 'workscout' ), '<span class="function">' . esc_html( $item['function'] ) . '</span>', '<span class="organisation">' . esc_html( $item['organisation'] ) . '</span>' ); ?></strong> 
						</dt>
						<dd>
